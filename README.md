...
O sistema operacional é um software que pode ser utilizado como base para qualquer computador ou celular, são responsáveis por **todo o funcionamento do dispositivo**, tanto como a interface entre o usuário e os dispositivos quanto os comandos de processamento, alguns exemplos famosos são os da **Windows**, **Google**, utilizado em Chromebooks, o *ChromeOS*, **Apple**, o *iOS* e *macOS* e o **Linux**. 
...
#### Windows
Criado pelos estudantes **Bill Gates** e **Paul Allen**, em um computador PDP-10 da Digital Equipment Corporation no ano de 1975, surgiu o **Windows**. Em 1985, foi lançado o **Windows 1.0**, que rodava sobre O *MS-DOS*, o software lançado era apenas uma interface gráfica. e não um sistema operacional, essa primeira versão não foi muito popular, só na **versão 3.x**, o sistema conseguiu evoluir e fazer sucesso, a partir disso, suas vendas começaram a crescer muito, dando origem a novas versões que nós conhecemos hoje em dia, como o *Windows 10*. O Windows é um **sistema operacional proprietário**, onde o código não é aberto à modificação de todos.
...
## Comandos do Windows
- Alguns dos comandos do **Prompt de Comando(CMD)** do Windows mais utilizados e suas respectivas funções são:
  - O comando **````mkdir````** **cria** uma nova pasta, exemplo ````mkdir NovaPasta```` 
  - O comando **````copy````** **copia** arquivos de um local para outro, exemplo ````copy C:\Arquivo.txt D:\Backup\````
  - O comando **````del````** **exclui** um arquivo ou mais, exemplo ````del meu_arquivo.txt````
  - O comando **````tasklist````** exibe todos os **processos em execução do sistema**, exemplo ````tasklist````
  - O comando **````ipconfig````** exibe informações de rede, como o **IP do computador**, exemplo ````ipconfig````
  - O comando **````ping````** testa a **conectividade** com **outro dispositivo ou site na rede**, exemplo ````ping www.google.com````
  - O comando **````systeminfo````** exibe **informações detalhadas** sobre o sistema operacional, exemplo ````systeminfo````
  - O comando **````chkdsk````** **verifica e corrige erros no disco rígido**, exemplo ````chkdsk C:````
  - O comando **````sfc/scannow````** **verifica e corrige arquivos corrompidos do sistema**, exemplo ````sfc/scannow````
  - O comando **````shutdown````** **desliga ou reinicia o computador**, exemplo desligando após 60 segundos ````shutdown/s/t 60````
...
- Aproximadamente **78%** das pessoas no mundo *utilizam o Windows*.
